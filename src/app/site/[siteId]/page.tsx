          ...
          <div className="bg-navy-850 rounded-xl border border-slate-800/50 p-6 card-glow">
            ...
            <div className="grid grid-cols-4 gap-2">
              {devices.slice(0, 20).map((device) => (
                <Link href={`/site/${siteId}/devices/${device.id}`} key={device.id} className="aspect-square bg-navy-900/50 rounded-lg border border-slate-800/30 flex flex-col items-center justify-center gap-0.5 hover:border-carbon-500/20 transition-all cursor-pointer group relative" title={`${device.name}\n${device.ip}\n${device.model}\nClients: ${device.clients}\nRole: ${device.role}`}>
